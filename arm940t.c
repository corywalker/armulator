...
instruction decode_instruction(arm940t * cpu, uint32_t word)
{
	instruction inst;
	inst.word = word;
	inst.cond = word >> 28;
    ...
    else if(((word_adj&0xFE000010)==0x00000000)||((word_adj&0xFE000090)==0x00000010)||
    		((word_adj&0xFE000000)==0x02000000)) { //Data Processing
		inst.group = 1;
		inst.char0 = get_bits(word, 21, 4); //opcode
		inst.char1 = get_bits(word, 20, 1); //S
		inst.char2 = get_bits(word, 16, 4); //Rn
		inst.char3 = get_bits(word, 12, 4); //Rd
...
void cycle(arm940t * cpu)
{
	if(cpu->have_decoded)
	{
		if(condition(cpu, cpu->decoded.cond)) // check if condition is met
		{
			...
			{
				case 1:
					run_opcode(cpu, cpu->decoded.char0, cpu->decoded.char1, cpu->decoded.char2, cpu->decoded.char3,
						addrmode1(cpu, cpu->decoded.word));
					break;
				case 2:
					if(cpu->decoded.char0)
						cpu->R[14] = cpu->R[15]-4;
	...
	if(cpu->have_fetched)
	{
		cpu->decoded = decode_instruction(cpu, cpu->fetched);
		cpu->have_decoded = 1;
		printf("0x%08X: ", cpu->R[15]-4);
		print_instruction(cpu->decoded);
	}
	if(cpu->R[15] < cpu->binarysz)
	{
		cpu->fetched = fetch_instruction(cpu, cpu->R[15]);
		cpu->have_fetched = 1;
	}
	else
		cpu->have_fetched = 0;
...
{
	uint32_t word = 0;
	if(address <= cpu->binarysz-4) //todo: hack
	{
		word += cpu->binary[address]; //converts from little endian to big endian
...
void print_instruction(instruction inst)
{
	switch (inst.group) {
	case 0:
		printf("Undefined group\n");
		break;
	case 1: { //data processing
		switch (inst.char0) { //opcode
		case 13:
			print_operation("mov", inst.cond, inst.char1);
			printf(" R%i, %u\n", inst.char3, inst.short0);
			break;
		case 4:
			print_operation("add", inst.cond, inst.char1);
			printf(" R%i, R%i, %u\n", inst.char3, inst.char2, inst.short0);
			break;
		case 10:
...
{
	cpu->memory = (uint8_t*) malloc (size+1);
	cpu->memorysz = size;
}
